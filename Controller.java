...
final class Controller {
    ...
    private Command[] _commands = {
            ...
            new Command("auto\\s+black|auto\\s+white$", this::doAuto),
            new Command("manual\\s+black|manual\\s+white$", this::doManual),
    ...
    private void doAuto(Matcher mat) {
        if (mat.group().toLowerCase() == "white") {
    ...
    private void doManual(Matcher mat) {
        if (mat.group().toLowerCase().equals("manual black")) {
            _black = _manualPlayerTemplate;
        } else {
            _white = _manualPlayerTemplate;
        }
    }
    ...
    private void doMove(Matcher mat) {
        Square from = sq(mat.group(1));
        Square to = sq(mat.group(2));
        Square spear = sq(mat.group(3));

        from = Square.sq(from.col(), from.row() - 1);
        to = Square.sq(to.col(), to.row() - 1);
        spear = Square.sq(spear.col(), spear.row() - 1);
        _board.makeMove(Move.mv(from, to, spear));
    }
